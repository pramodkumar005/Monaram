...
const BecomePartner = (props) => {
  ...
  console.log('Action.currentScene>>>>>>props>>>>>>>>>>>>'+props.lastPage)
  ...
  const addPartnerRequest=()=>{
    ...
    var data = {name: clinicName, address:address, addressCont:addressCont, city:selectedValue, state: stateValue, pin:pin, phone:phoneNumber, proofType: selectedID, createdBy: currentUser.uid, requestDate: moment().unix(), status:"review", bankName: bankName, accountNumber:accountNumber, ifsc: ifscCode, upi: upid}
...
//------------upload photo-------------------
const uploadUserPhoto=(partnerId)=>{
  ...
  const updateUserPhotoToPartner = async (partnerId)=>{
    ...
    .then((response) => {
      Helper.getNotificationToAdminListForCenter(clinicName)
  ...
  const getUserPhotoDownloadUrl = async(partnerId)=>{
      ...
      {processing?
        ...
        <TouchableOpacity style={{width:'100%', backgroundColor:global.background, position:'absolute', bottom:0, height:60, justifyContent:'center', alignItems:"center"}}  onPress={()=>{validateForm()}}>
          <View style={{width:'90%', backgroundColor:global.primary, height:50, justifyContent:'center', alignItems:"center", borderRadius:5}}>
            <Text style={{color:global.white, fontSize:18, fontWeight:'bold'}}>REGISTER</Text>
          </View>
        </TouchableOpacity>      
      }
